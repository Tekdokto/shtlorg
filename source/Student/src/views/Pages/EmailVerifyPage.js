          ...
          <form>
            <Card login >
              <CardHeader className={`${classes.cardHeader} ${classes.logCardHeader} ${classes.textCenter}`} >
                  ...
                  {(this.props.verify_email_error != "")?this.props.verify_email_error:"Verifying email..."}
                  </h4>                  
              </CardHeader>             
              <CardBody>
...
const mapPropsToState = (state) => {
  // console.log("VALUE:",state.authReducer)
 return {
  verify_email_error : state.authReducer.verify_email_error
 }
}

const mapDispatchToProps = { resetpasswordUser,resetNotification,verifyEmail };

export default connect(mapPropsToState, mapDispatchToProps)(withStyles(styles)(EmailVerifyPage));