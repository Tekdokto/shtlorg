...
class SpecSheet extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (
      props.user.student_details &&
      props.user.student_work_history &&
      props.user.student_education && 
      props.user.student_attachment
    ) {      
      return {
        ...
        student_details: props.user
          ? props.user.student_details.length > 0
            ? props.user.student_details
            : [state.student_details_default_obj]
          : [state.student_details_default_obj],
        student_work_history: props.user
          ? props.user.student_work_history.length > 0
            ? props.user.student_work_history
            : state.student_work_history
          : state.student_work_history,
        student_education: props.user
          ? props.user.student_education.length > 0
            ? props.user.student_education
            : state.student_education
          : state.student_education,
        student_attachment: props.user
          ? props.user.student_attachment.length > 0
            ? props.user.student_attachment
            : state.student_attachment
          : state.student_attachment,
      };
    }
      ...
      if (this.checkStudentDetails(this.state.student_details)) {
        if (this.checkWorkHistory(this.state.student_work_history)) {
          if (this.checkOtherDetails(this.state.student_education)) {
            ...
            if(this.state.student_details[0].id){
              update_obj.append("id", `${this.state.student_details[0].id}`)
            }
            update_obj.append("user_id", `${this.props.user.user_id}`);
            ...
            const response = await axios.post(`${API_URL}/student/update_specsheet`, update_obj, headers)
            ...
            // this.props.editSpecSheetData(update_obj, this.props.history);
                  ...
                  tabContent: (
                    <Card className="paddingTopBottom cardCustom">
                      ...
                      {/* <GridContainer spacing={6}>
                            <GridItem xs={12} md={10} className="has-btn-title">
                              Edit Work History
                            </GridItem>
                            <GridItem xs={12} md={2} className={classes.rightLeftResponsiveMdDown}>
                              <Button
                                color="info"
                                size="lg"
                      ...
                      {/* <GridContainer spacing={6}>
                          ...
                          <GridItem xs={12} md={2} className={classes.rightLeftResponsiveMdDown}>
                            <Button
                              ...
                              size="sm"
                              className={`${classes.newButton} ${classes.mb30} ${classes.mt15}`}
                              onClick={(e) => { this.addNewOtherDetail() }}
                            >
                              Add
                                </Button>
                          </GridItem>
                ...
                },
                {
                  ...
                  tabContent: (
                    <Card className="paddingTopBottom cardCustom">
                      ...
                      <CardBody className="cardCustomBody my-resume">
                        <Grid
                          container
                          className={`${classes.root} ${classes.mb30}`}
                          spacing={4}
