...
class IntroDiscoveryCallList extends React.Component {
  ...
  setInterviewModal(val = false, confirm_id = 0, student_id = 0, student_name = "", watchlistname = "", interview_date = "", interview_time = "",is_got_other_company_offer=0) {
  ...
  async getData(state) {
    ...
    const res = await axios.post(`${API_URL}/company/watchlist/get_onsite_intro_discovery_call_list`, get_watchlist, headers);
      ...
      return {
        ...
        actions: (
          ...
          <div className="actions-right">
            ...
            <CustomTooltip title="Edit" position="left"><IconF icon={circleEditOutline} style={{ "cursor": "pointer" }} onClick={() => this.setInterviewModal(true, prop['id'], prop['student_id'], prop['candidatename'], prop['watchlistname'], prop['intro_discovery_response_on_date'], prop['intro_discovery_response_on_time'],prop['is_got_other_company_offer'])} /></CustomTooltip>{" "}
            <CustomTooltip title="Pass" position="left"><PlaylistAddCheckIcon style={{ "cursor": "pointer" }} onClick={() => this.passOnsiteIntroList({ id: prop['id'], student_id: prop['student_id'], status: 1 },prop['is_got_other_company_offer'])} /></CustomTooltip>{" "}
  ...
  passOnsiteIntroList(params,is_got_other_company_offer=0) {
  ...
  editOnsiteIntroList() {
    ...
    // console.log("DAte diff:", moment(temp_date).diff(moment(current_date, ["YYYY-MM-DD"]), 'days'))
    // console.log("PASS", temp_date, moment(this.state.onsiteinterviewdate, ["MM-DD-YYYY"]).format("MM-DD-YYYY"), moment().diff(moment(temp_date, ["YYYY-MM-DD"]), 'days'), this.state.onsiteinterviewdate && this.state.onsiteinterviewdate !== "" && moment(this.state.onsiteinterviewdate).format("MM-DD-YYYY") !== "Invalid date" && moment().diff(moment(this.state.onsiteinterviewdate).format("MM-DD-YYYY"), 'days') > 0 && this.state.onsiteinterviewtime && this.state.onsiteinterviewtime !== "")
    if (this.state.onsiteinterviewdate && this.state.onsiteinterviewdate !== "" && temp_date !== "Invalid date" && moment(temp_date).diff(moment(current_date, ["YYYY-MM-DD"]), 'days') > 0 && this.state.onsiteinterviewtime && this.state.onsiteinterviewtime !== "") {
      ...
      // console.log("Fail", temp_date !== "Invalid date" && moment(temp_date).diff(moment(current_date, ["YYYY-MM-DD"]), 'days') <= 0)
  ...
  async callPassFailIntroList(params) {
    ...
    let token = (User) ? ((User.token) ? User.token : '') : '';
    let headers = { headers: { token: `${token}` } }
    const res = await axios.post(`${API_URL}/company/watchlist/pass_fail_onsite_intro_discovery_call`, params, headers)
    return await res.data;
  }
  ...
  render() {
    ...
    return (
      <div className="main-right-panel">
        <GridContainer>
          <GridItem xs={12} sm={8}>
            <h1>Onsite Intro Discovery Call List</h1>
              ...
              >
                ...
                >
                  <Button
                    ...
                    size="lg"
                    className={`${classes.blockButton} ${classes.mt30} ${classes.mb30}`}
                    onClick={(e) => this.editOnsiteIntroList()}
                  >
                    Submit
                </Button>
                </DialogActions>
